...
INPUT:  1) spInput: Spectral type to select (e.g. L0); it can also be a single object, identified by unum (e.g. U20268).
        ...
        5) std: Boolean, whether to get the NIR standard spectrum
        6) lbl: Boolean, whether to get the labels of the individual spectra
...
# specData and limits must be dictionaries.
    ...
    for bandIdx, band in enumerate(bandNames):
        ...
        if plotInstructions is not None:
            ...
            tmpStd  = numpy.where(numpy.array(plotInstructions) == 'standard')
            numFld  = len(tmpFld[0])
            numYng  = len(tmpYng[0])
            numStd  = len(tmpStd[0])
